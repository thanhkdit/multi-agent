  ...
  installGraphqlListener(page) {
    ...
    page.on('response', async (response) => {
      try {
        ...
        // Save to file in debug folder
        try {
          let friendlyName = 'graphql_query';
          ...
          if (match) {
            ...
            try {
              ...
              if (parsedPostData && parsedPostData.fb_api_req_friendly_name) {
                friendlyName = parsedPostData.fb_api_req_friendly_name;
              }
            } catch (_) {}
          }
          const timestamp = Date.now();
          const rand = Math.floor(Math.random() * 10000);
          const fileName = `graphql_${timestamp}_${rand}_${friendlyName}.json`;
          ...
          // ignore error to prevent crash during scraping
        }

...
- **Gọi Lần 1:** Delegate `agent-scraper` với payload `{script: "facebook_ads_library.js", params: ["<tên_page>", "<limit>", "<tên_đối_thủ>"]}`. (Ví dụ chuẩn: `params: ["TPBank", "5", "TPBank"]`).
- *Chờ kết quả.*
- **Gọi Lần 2:** Delegate `agent-scraper` với payload `{script: "facebook_feed.js", params: ["<url_page>", "<limit>", "<tên_đối_thủ>"]}`. (Ví dụ chuẩn: `params: ["https://www.facebook.com/TPBank./", "6", "TPBank"]`. Lưu ý: tự suy nghĩ limit phù hợp, nếu user không chỉ định, tối đa là 6).
- *Chờ kết quả.*
- **Gọi Lần 3:** Delegate `agent-scraper` với payload `{script: "video_transcript.py", params: ["<url_video>", "<url_video>", "<url_video>", ...]}`.
- *Chờ kết quả.*
- **Gọi Lần 4 (Nếu hỏi về kênh TikTok):** Delegate `agent-scraper` với payload `{script: "tiktok/analytic.js", params: ["<uniqueId>", "<tên_đối_thủ>"]}`. (Ví dụ chuẩn: `params: ["realpewpew", "PewPew"]`).
